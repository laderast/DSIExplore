...
question("What can we learn from this table?",
  answer("There are more alive people than dead people in the data.", correct=TRUE, message="Not correct."),
  answer("There are two variables", message="Not quite. There's `outcome`, `smoker`, and `age`."),
  answer("You are either considered either a smoker or a non-smoker", correct=TRUE, message="Yes, that's correct!"),
  answer("The age range of the patients includes minors.", message="Look at the min value for age."),
  answer("The average age of the participant was around 47 years of age", correct="Yes, you can see it as the mean value for the age" )
)
```
...
Let's ask the question again, with a younger group: are smokers under 45 more likely to die than non smokers?

```{r}
sliderInput("slidy2", "Age Cutoff", min=min(Whickham$age)+1, max=max(Whickham$age)-1, value=c(18, 83),step = 5)
