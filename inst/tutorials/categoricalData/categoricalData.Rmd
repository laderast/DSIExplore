...
```{r}
sliderInput("slide_chi", "slide to adjust statistical cutoff", value=0.8, min = 0.1, max=10)
plotOutput("chihist")
```
...
output$chihist <- renderPlot({
  ...
  data.frame(chiveq, GT=ifelse(chiveq >= chival, "> chisq", "< chisq")) %>% 
    ggplot(aes(x=chiveq, fill=GT)) + geom_histogram(bins=60) + geom_density() +
...
Our chi-squared distribution can be calculated
...
One thing our test statistic is dependent on is *n*, the total number of subjects in the study. We need to be cautious when our *n* is very small (less than 10), or when we have less than 5 subjects in each cell of our table.
