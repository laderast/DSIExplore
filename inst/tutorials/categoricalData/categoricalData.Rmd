...
vis1 <- tabNoAssoc %>% ggplot(aes(x=outcome, y=smoking)) + geom_tile(fill="white", colour="black") + 
  geom_text(aes(label=value)) 

vis2 <- noAssocSmoking %>% ggplot(aes(x=outcome, fill=smoking)) + geom_bar(color="black",position = "fill") 
...
In other words, how likely is the association between smoking and outcome just something that we could observe by chance?

Our test statistic is known as the *Chi-Squared-statistic*. 
...
We're going to do a little calculation using R. Try running the code. How have we filtered the data? Change the filtering criteria to age < 40 and see how the table changes.
...
for(i in 1:nTrials){
  smoking = ifelse(rbinom(sampSize, 1, 0.3)==0, "No","Yes")
  outcome = ifelse(rbinom(sampSize, 1, 0.2)==0, "Alive", "Dead")
...
Before we even calculate a p-value, we must decide on a level of significance. Setting this value before we test is important because we specify 
...
[Missing data explorer] <- use proportions to suss out whether there are biases in data.
