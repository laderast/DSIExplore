...
The T-test is pretty robust to slight violations of the normality assumption, especially since we have a large sample size
...
- To assess continuous factors we can make scatterplots
- We can stratify scatterplots by categorical variables to assess associations of continuous variables with BMI within subcategories defined by categorical or binary variables (i.e. `Diabetes`)
- To remove the stratification option select `AllSubjects` as your `Factor`
- **Question: Do any interesting associations arise?**

```{r}
cat_vars = c("AllSubjects","Gender","Diabetes","Smoke100n","Race1","Race3","PhysActive","TVHrsDay","SleepTrouble")
...
output$pointPlot1 <- renderPlot(
  nhanes_filtered %>% ggplot(aes(x=get(input$select_cont1),y=BMI, color=get(input$select_cont1))) + geom_point(alpha=0.1) + facet_wrap(~get(input$select_factor1)) + 
    xlab(input$select_cont1) + geom_smooth(method="lm")
)
...
Often we believe our measure or ourcome of interest (i.e. BMI) is associated with several other variables at once. For instance, BMI could be associated with diabetes, blood pressure, physical activity, diet, among many other characteristics.
...
Mathematically, we are modeling our outcome as a *linear* combination of our other variables.
- assumption: plus some random error, call it $\ \epsilon$ that is normally distributed
...
We can run a linear regression ini `R` and view the output:
...
This idea of "how well can we predict our outcome" drives much of statistical modeling, inclduing **machine learning** and **predictive analytics**.
...
Modify the R code in the `lm()` function to build your own prediction model. Here are the names of the NHANES dataset:
...
nhanes_filtered %>% ggplot(aes(x=Weight,y=BMI,color=Diabetes)) + geom_point() + geom_smooth(method="lm")
