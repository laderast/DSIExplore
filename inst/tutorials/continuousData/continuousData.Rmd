...
# need to figure out better way to get and then use labels as straight strings

plot_w_cor_quotes = function(df,cont_x,cont_y,color,xlab,ylab,colorlab) {
  enquo_x = enquo(cont_x)
  enquo_y = enquo(cont_y)
  enquo_color = enquo(color)
  tmpdat =  df%>%summarize(x=mean(range(!!enquo_x,na.rm=T)),
  ...
  df %>% ggplot(aes_(x=enquo_x,y=enquo_y,color=enquo_color)) + geom_point() + stat_smooth(method="lm",se = FALSE) + 
    geom_text(data = tmpdat,aes(x=x,y=y,label=cortext),inherit.aes = FALSE,size=4)+xlab(xlab)+ylab(ylab)+
    scale_color_continuous(name=colorlab)
}



  output$scatterPlotCont <- renderPlot(
      NHANES %>% plot_w_cor_quotes(get(input$select_x),get(input$select_y),get(input$select_color),input$select_x,input$select_y,input$select_color)
      )
